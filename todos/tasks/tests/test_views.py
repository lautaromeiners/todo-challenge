...
class TaskViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = get_user_model().objects.create_user(
            email="testuser@example.com", username="test", password="aStrongPassword"
        )
        self.user.save()
    ...
    def test_task_belonging_to_user(self):
        ...
        another_user.save()
        task_for_another_user = Task.objects.create(
            user=another_user,
            title="Another Task",
            description="A task for another user",
        )
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertNotIn(
            task_for_another_user.id, [task["id"] for task in response.data]
        )
        self.assertEqual(
            len(response.data), 3
        )  # Same three tasks as before, no new task
    ...
    def test_filter_by_status_completed(self):
        ...
        self.assertTrue(
            all(task["status"] == TaskStatus.COMPLETED for task in response.data)
        )
    ...
    def test_filter_by_status_open(self):
        ...
        self.assertEqual(len(response.data), 2)
        self.assertTrue(
            all(task["status"] == TaskStatus.OPEN for task in response.data)
        )
